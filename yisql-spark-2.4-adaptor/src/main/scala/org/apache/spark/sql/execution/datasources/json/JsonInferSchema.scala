...
object JsonInferSchema {
  ...
  def inferJson(json: Seq[UTF8String], sparkSession: SparkSession): StructType = {
  ...
  def createDataSet(json: Seq[String], sparkSession: SparkSession): DataFrame = {
    val parsedOptions = new JSONOptions(
      Map[String, String](),
      sparkSession.sessionState.conf.sessionLocalTimeZone,
      sparkSession.sessionState.conf.columnNameOfCorruptRecord)
    val schema: StructType = infer(json.take(1).map(UTF8String.fromString), parsedOptions, CreateJacksonParser.utf8String)
    val createParser: (JsonFactory, String) => JsonParser = CreateJacksonParser.string
    val rawParser = new JacksonParser(schema, parsedOptions)
    val parser = new FailureSafeParser[String](
      (input: String) => rawParser.parse(input, createParser, UTF8String.fromString),
      parsedOptions.parseMode,
      schema,
      parsedOptions.columnNameOfCorruptRecord,
      false)
    val parsed: Seq[InternalRow] = json.map { row: String =>
      parser.parse(row).next()
    }
    val attributes: Seq[AttributeReference] = schema.toAttributes
    //val plan = new WowFastLocalRelation(attributes, parsed)
    val plan: LocalRelation = LocalRelation(attributes, parsed)
    Dataset.ofRows(sparkSession, plan)
  }

  def toJson(data: Iterator[Row], rowSchema: StructType, sessionLocalTimeZone: String, callback: String => Unit): Unit = {
    val writer = new CharArrayWriter()
    // create the Generator without separator inserted between 2 records
    val gen = new JacksonGenerator(rowSchema, writer,
      new JSONOptions(Map.empty[String, String], sessionLocalTimeZone))
    val enconder: ExpressionEncoder[Row] = RowEncoder.apply(rowSchema).resolveAndBind()
  ...
  def getJsonGen(rowSchema: StructType, sessionLocalTimeZone: String): (JacksonGenerator, CharArrayWriter) = {
    val writer = new CharArrayWriter()
    val gen = new JacksonGenerator(rowSchema, writer,
      new JSONOptions(Map.empty[String, String], sessionLocalTimeZone))
    (gen, writer)
  ...
  def toJson(dataSet: DataFrame): Array[String] = {
    val rowSchema: StructType = dataSet.schema
    val sessionLocalTimeZone: String = dataSet.sparkSession.sessionState.conf.sessionLocalTimeZone
    ...
    val enconder: ExpressionEncoder[Row] = RowEncoder.apply(rowSchema).resolveAndBind()
    val res: Array[String] = dataSet.collect.map { row: Row =>
      gen.write(enconder.toRow(row))
      gen.flush()
      val json: String = writer.toString
      writer.reset()
  ...
  def infer[T](
    ...
    val parseMode: ParseMode = configOptions.parseMode
    val columnNameOfCorruptRecord: String = configOptions.columnNameOfCorruptRecord

    // perform schema inference on each row and merge afterwards
    val factory = new JsonFactory()
    configOptions.setJacksonOptions(factory)

    val rootType: DataType = json.map { row: T =>
      try {
        Utils.tryWithResource(createParser(factory, row)) { parser: JsonParser =>
          parser.nextToken()
    ...
    }.filter((_: Option[DataType]).isDefined).map((_: Option[DataType]).get).fold(StructType(Nil))(compatibleRootType(columnNameOfCorruptRecord, parseMode))
  ...
  private[this] val structFieldComparator: Comparator[StructField] = new Comparator[StructField] {
  ...
  private def inferField(parser: JsonParser, configOptions: JSONOptions): DataType = {
    ...
    parser.getCurrentToken match {
      ...
      case START_OBJECT =>
        val builder: mutable.ArrayBuilder[StructField] = Array.newBuilder[StructField]
      ...
      case VALUE_NUMBER_INT | VALUE_NUMBER_FLOAT =>
        ...
        parser.getNumberType match {
          ...
          case FLOAT | DOUBLE if configOptions.prefersDecimal =>
            val v: java.math.BigDecimal = parser.getDecimalValue
            if (Math.max(v.precision(), v.scale()) <= DecimalType.MAX_PRECISION) {
              DecimalType(Math.max(v.precision(), v.scale()), v.scale())
            } else {
              DoubleType
    ...
    TypeCoercion.findTightestCommonType(t1, t2).getOrElse {
      ...
      (t1, t2) match {
        ...
        case (t1: DecimalType, t2: DecimalType) =>
          val scale: Int = math.max(t1.scale, t2.scale)
          val range: Int = math.max(t1.precision - t1.scale, t2.precision - t2.scale)
          if (range + scale > 38) {
        ...
        case (StructType(fields1), StructType(fields2)) =>
          ...
          while (f1Idx < fields1.length && f2Idx < fields2.length) {
            val f1Name: String = fields1(f1Idx).name
            val f2Name: String = fields2(f2Idx).name
            val comp: Int = f1Name.compareTo(f2Name)
            if (comp == 0) {
              val dataType: DataType = compatibleType(fields1(f1Idx).dataType, fields2(f2Idx).dataType)
