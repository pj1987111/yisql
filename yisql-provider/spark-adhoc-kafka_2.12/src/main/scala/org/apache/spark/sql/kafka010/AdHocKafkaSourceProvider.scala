...
class AdHocKafkaSourceProvider extends KafkaSourceProvider {
    ...
    override def createRelation(
        ...
        validateBatchOptions(caseInsensitiveMap)
        ...
        val startingRelationOffsets = KafkaSourceProvider.getKafkaOffsetRangeLimit(
            caseInsensitiveMap, STARTING_OFFSETS_BY_TIMESTAMP_OPTION_KEY, STARTING_OFFSETS_OPTION_KEY, EarliestOffsetRangeLimit)
        assert(startingRelationOffsets != LatestOffsetRangeLimit)

        val endingRelationOffsets = KafkaSourceProvider.getKafkaOffsetRangeLimit(
            caseInsensitiveMap, ENDING_OFFSETS_BY_TIMESTAMP_OPTION_KEY, ENDING_OFFSETS_OPTION_KEY, LatestOffsetRangeLimit)
        assert(endingRelationOffsets != EarliestOffsetRangeLimit)

        new AdHocKafkaRelation(
            sqlContext,
            strategy(caseInsensitiveMap),
            sourceOptions = caseInsensitiveMap,
            specifiedKafkaParams = specifiedKafkaParams,
            failOnDataLoss = failOnDataLoss(caseInsensitiveMap),
    ...
    private def validateBatchOptions(caseInsensitiveParams: CaseInsensitiveMap[String]) = {
        ...
        KafkaSourceProvider.getKafkaOffsetRangeLimit(
            caseInsensitiveParams, STARTING_OFFSETS_BY_TIMESTAMP_OPTION_KEY, STARTING_OFFSETS_OPTION_KEY, EarliestOffsetRangeLimit) match {
        ...
        KafkaSourceProvider.getKafkaOffsetRangeLimit(
            caseInsensitiveParams, ENDING_OFFSETS_BY_TIMESTAMP_OPTION_KEY, ENDING_OFFSETS_OPTION_KEY, LatestOffsetRangeLimit) match {
