...
import com.zhy.yisql.addon.cmd.hdfs.HdfsFsShell
...
class HDFSCommand extends SQLCmd {
  override def run(spark: SparkSession, path: String, params: Map[String, String]): DataFrame = {
    val conf: Configuration = spark.sessionState.newHadoopConf()
    val args: Seq[String] = JSONTool.parseJson[List[String]](params("parameters"))
    conf.setQuietMode(false)
    var output = ""
    val fsShell = new HdfsFsShell(conf, path)
    try {
    ...
    if (args.contains("-F")) {
      val ds: Dataset[String] = spark.createDataset(output.split("\n").toSeq)
      spark.read.json(ds)
    } else {
