...
object TemplateMerge {
  def merge(sql: String, root: Map[String, String]): String = {
    ...
    val predified_variables: Map[String, String] = Map[String, String](
    ...
    val newRoot:Map[String, AnyRef] = Map("date" -> new DateTime()) ++ predified_variables ++ root
    val wow: String = RenderEngine.namedEvaluate(sql, newRoot)
    wow
  }
}
