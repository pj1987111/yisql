...
class RunAdaptor(scriptSQLExecListener: ScriptSQLExecListener) extends DslAdaptor {
  ...
  override def parse(ctx: SqlContext): Unit = {
    ...
    val runDf: DataFrame = RunAdaptor.findAlg(format).run(scriptSQLExecListener.sparkSession, path, options)

    val tempTable: String = if (asTableName.isEmpty) UUID.randomUUID().toString.replace("-", "") else asTableName
...
object RunAdaptor {
  def findAlg(name: String): SQLCmd = {
