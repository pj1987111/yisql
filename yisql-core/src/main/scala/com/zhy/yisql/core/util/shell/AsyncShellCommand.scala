package com.zhy.yisql.core.util.shell
...
import com.zhy.yisql.core.util.YiSQLUtils
...
class AsyncShellCommand(user: String, command: String, token: String, timeout: Long, isInteractive: Boolean = false)
        extends TFileWriter {

    @transient private lazy val logger = LogManager.getLogger(classOf[AsyncShellCommand])

    val taskId = YiSQLUtils.getUUID32 + "_" + token
    val homeDir = s"/tmp/yisql/${taskId}"
    ...
    schedule.schedule(new Runnable {
        override def run(): Unit = {
            try {
                ...
                case e: Exception =>
                    logger.info("schedule to clean up tasks expired without any operation in 3 hours failed,please check error log", e)
            }
        }
