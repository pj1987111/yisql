...
class PlatformManager extends Logging {
  ...
  val lifeCycleCallback: ArrayBuffer[PlatformLifecycle] = ArrayBuffer[PlatformLifecycle]()
  //    var zk: ZKClient = _

  def register(listener: PlatformManagerListener): Unit = {
    listeners += listener
  }

  def unRegister(listener: PlatformManagerListener): Unit = {
    listeners -= listener
  }

  def registerPlatformLifecycle(listener: PlatformLifecycle):Unit = {
    lifeCycleCallback += listener
  }

  def unRegisterPlatformLifecycle(listener: PlatformLifecycle):Unit = {
    lifeCycleCallback -= listener
  }

  def startRestServer(runtime: StreamingRuntime): Unit = {
    runtime.startHttpServer()
  }

  def startThriftServer(runtime: StreamingRuntime): Unit = {
  ...
  def run(_params: ParamsUtil, reRun: Boolean = false): Unit = {

    if (!reRun) {
      config.set(_params)
    }

    val params: ParamsUtil = config.get()

    val lastStreamingRuntimeInfo = if (reRun) {
      val tempRuntime: StreamingRuntime = PlatformManager.getRuntime
    ...
    params.getParamsMap.asScala.filter((f: (String, String)) => f._1.startsWith("streaming.")).foreach { f: (String, String) => tempParams.put(f._1, f._2) }


    TryTool.tryLogNonFatalError {
      lifeCycleCallback.foreach((f: PlatformLifecycle) => f.beforeRuntime(params.getParamsMap.asScala.toMap))
    }

    val runtime: StreamingRuntime = PlatformManager.getRuntime

    TryTool.tryLogNonFatalError {
      lifeCycleCallback.foreach((f: PlatformLifecycle) => f.afterRuntime(runtime, params.getParamsMap.asScala.toMap))
    }
    ...
    TryTool.tryLogNonFatalError {
      lifeCycleCallback.foreach((f: PlatformLifecycle) => f.beforeDispatcher(runtime, params.getParamsMap.asScala.toMap))
    }
    ...
    TryTool.tryLogNonFatalError {
      lifeCycleCallback.foreach((f: PlatformLifecycle) => f.afterDispatcher(runtime, params.getParamsMap.asScala.toMap))
    }
...
object PlatformManager {
  ...
  def createRuntimeByPlatform(name: String, tempParams: java.util.Map[Any, Any]): StreamingRuntime = {
    Class.forName(name).
  ...
  def getRuntime: StreamingRuntime = {
    ...
    params.asScala.map((f: (String, String)) => tempParams.put(f._1.asInstanceOf[Any], f._2.asInstanceOf[Any]))

    val platformName: String = params.getOrDefault("streaming.platform", SPARK)
    val runtime: StreamingRuntime = createRuntimeByPlatform(platformNameMapping(platformName), tempParams)
  ...
  def platformNameMapping: Map[String, String] = Map[String, String](
