...
class SQLResource (spark: SparkSession, owner: String, getGroupId: String => String) {
    ...
    def resourceSummary(jobGroupId: String) = {
        val store = SQLUtils.getAppStatusStore(spark)
        ...
        val shuffle = SQLShufflePerfRender(memoryBytesSpilled = currentMemoryBytesSpilled, diskBytesSpilled = currentDiskBytesSpilled, inputRecords = currentInputRecords)
