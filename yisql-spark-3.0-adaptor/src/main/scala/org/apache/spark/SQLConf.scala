...
object SQLConf {
    ...
    val SQL_ENABLE_REST = SQLConfigBuilder("streaming.rest").doc(
        """
          |enable rest api
        """.stripMargin).booleanConf.createWithDefault(false)

    val SQL_DRIVER_PORT = SQLConfigBuilder("streaming.driver.port").doc(
        """
          | The port of rest api
        """.stripMargin).intConf.createWithDefault(9003)


    val SQL_MASTER = SQLConfigBuilder("streaming.master")
    ...
    val SQL_HIVE_CONNECTION = SQLConfigBuilder("streaming.hive.javax.jdo.option.ConnectionURL").doc(
        """
    ...
    val SQL_CARBONDATA_STORE = SQLConfigBuilder("streaming.carbondata.store").doc(
        """
          |
    """.stripMargin).stringConf.createOptional

    val SQL_CARBONDATA_META = SQLConfigBuilder("streaming.carbondata.meta").doc(
        """
          |
    """.stripMargin).stringConf.createOptional
    ...
    val SQL_DISABLE_SPARK_LOG = SQLConfigBuilder("streaming.disableSparkLog").doc(
        """
          |Sometimes there are too much spark job info, you can disable them.
        """.stripMargin).booleanConf.createWithDefault(false)

    val SQL_UDF_CLZZNAMES = SQLConfigBuilder("streaming.udf.clzznames").doc(
        """
          |register udf class
        """.stripMargin).stringConf.createOptional
